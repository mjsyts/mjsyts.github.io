    ...
    params.forEach(param => {
        ...
        const valueToSlider = (value) => {
            if (param.curve === 'exp') {
                ...
                return normalized > 0 ? Math.log(normalized + 0.001) / Math.log(1.001) : 0;
            } else {
        ...
        const sliderToValue = (sliderVal) => {
            const normalized = sliderVal / 1000;
            if (param.curve === 'exp') {
                const expValue = Math.pow(1.001, normalized) - 0.001;
                return param.min + expValue * (param.max - param.min);
            } else {
                return param.min + normalized * (param.max - param.min);
            }
        };

        slider.value = valueToSlider(param.default);
        ...
        slider.addEventListener('input', (e) => {
            const value = sliderToValue(parseFloat(e.target.value));
