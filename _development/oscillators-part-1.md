...
In discrete time, we quantify the **period** of an oscillator in samples. For an exactly periodic discrete-time oscillator, **frequency** is a derived quantity: $$ f = Fs / N $$ As a consequence, not all frequency values are perfectly representable in digital audio. Exact cyclic repetition requires the period N to be an integer number of samples. This means that only frequencies of the form `Fs / N` can repeat exactly. For many frequencies, `Fs / f` is not an integer. In those cases, the signal can get very close to the desired frequency, but it cannot repeat with perfect sample-level periodicity. This limitation follows directly from discrete time and does not depend on the waveform or implementation. 
...
Phase is not a waveform and it is not a sound. It is a bookkeeping variable that tracks progress through a repeating cycle. In the simplest case, phase advances steadily from the start of the cycle to the end, then wraps around and repeats. The separation of **phase as position** and **waveform as shape** will be central throughout the rest of the series. 
